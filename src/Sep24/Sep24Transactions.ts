...
export class Sep24Transactions {
  ...
  getEnabledAssets({ operation }: { operation: "deposit" | "withdraw" }): Record<string, AssetDetails> {
    if (!this.isInitialized) {
      throw new Error("Initilaized first");
    }
  ...
  getAssetLimits({ assetCode, operation }: AssetInfoParams): { min: BigNumber; max: BigNumber } | null {
    if (!this.isInitialized) {
      throw new Error("Initilaized first");
    }
    const asset = this.supportedAssets[operation][assetCode.toUpperCase()];
    return asset?.enabled ? { min: asset.minAmount, max: asset.maxAmount } : null;
  }
